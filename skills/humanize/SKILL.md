---
name: humanize
description: Make written output read like a person wrote it, not a language model — stripping AI tells like em-dash overuse, buzzwords, and robotic parallelism. Use when producing any client-facing writing (bids, cover letters, resumes, outreach messages) that must not look AI-generated, or when the user asks to "humanize" or de-AI a draft.
---

# Humanize

Make the text read like a person wrote it, not a language model. Clients, recruiters,
and hiring managers actively screen out AI-written material — so the output must not
carry the usual AI fingerprints. When the user has their own winning samples or prior
writing, match *that* natural voice, not a generic "assistant" voice.

Apply these rules to whatever is being written or rewritten.

## Avoid these AI tells

- **Em-dash / hyphen overuse.** Do not pepper the text with `—`. A human bid uses few
...
Say the concrete thing (the actual tech, the actual result) instead of an abstract
claim. When in doubt, write it the way the user's winning samples would, in the chosen
voice.
